...
// função que converte uma string para maiusculo
...
// função que retorna a porcentagem de pessoas confirmadas internadas
...
int main(void)
{
  ...
  char nomePasta[10], municipio[30];
  int NCasos, N;
  ...
  // lendo nome da pasta
  scanf("%s", nomePasta);
  // lendo quantidade para ordem alfabetica
  scanf("%d", &NCasos);
  ...
  // criando comando para criar o diretorio
  ...
  // criando endereco para a pasta de output
  strcat(enderecoParaOutputItem3, "item_3.txt");
  strcat(enderecoParaOutputItem4, "item_4.txt");
  strcat(enderecoParaOutputItem5, "item_5.txt");
  strcat(enderecoParaOutputItem6, "item_6.txt");
  strcat(enderecoParaOutputItem7, "item_7.txt");
  ...
  // listando os municipios com mais de N casos
  ...
  imprimePorcentagensItem6(item6, municipio);
...
void mostraTotalCasosEntreDatas(FILE *arquivo, Data dataInicial, Data dataFinal)
{
  ...
  fprintf(arquivo, "- Total de pessoas: %d\n", totalCasos);
}
...
void listaCasosEmOrdemAlfabetica(FILE *arquivo, int n)
{
  ...
  // percorrendo a matriz com os nomes das cidades, que já está em ordem alfabetica
  for (i = 0; i < 78; i++)
  {
  ...
  for(i = 0; i < 77; i++)
  {
    for(j = 0; j < 76-i; j++)
    {
      ...
      if(vetorCasosPorMunicipio[j].numeroDeCasos < vetorCasosPorMunicipio[j+1].numeroDeCasos)
      {
        ...
        // município do vetor na posição j recebe o proximo municipio 
...
void listaTopNCidades(FILE *arquivo, int n, Data dataInicial, Data dataFinal)
{
  // inicializando variáveis 
  int i;
  // criando vetor local para analise dos casos por municipio
  CasosMunicipio vetorCasosPorMunicipio[78];

  // percorrendo a matriz municipios
  for (i = 0; i < 78; i++)
  {
...
void converteStringMaiusculo(char string[])
{
  ...
  {
    // convertendo letra para maiusculo
...
int ehMesmaData(Data data1, Data data2)
{
  // se os dias e meses da data inicial e final formm iguais
...
float calculaDesvioPradraoEntreDatas(Data dataInicial, Data dataFinal)
{
  ...
  // calculando média
  media = calculaMediaDeIdadeEntreDatas(dataInicial, dataFinal);
  ...
  for (dataInicial; !ehMesmaData(dataInicial, dataFinal); dataInicial = proximaData(dataInicial))
  {
    ...
    for (i = 0; i < TAMANHOCSV; i++)
    {
      ...
      if (ehMesmaData(vetorCsv[i].dataCadastro, dataInicial))
      {
        ...
        if (vetorCsv[i].classificacao)
        {
          // verificadno se o indivíduo veio a obito 
