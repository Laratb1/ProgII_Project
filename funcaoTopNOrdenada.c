...
void colocaEmOrdemDecrescente(CasosMunicipio vetorMunicipios[], int N);
...
int main(void)
{
  ...
  //mostraTotalCasosEntreDatas(data1, data2);
  // listando os municipios com mais de N casos
  //listaCasosEmOrdemAlfabetica(500);
  // mostrando csv
  //mostraCsv();

  listaTopNCidades(10, data1, data2);
...
void listaTopNCidades(int n, Data data1, Data data2)
{
  ...
  // percorrendo a matriz com os municipios
  for (i = 0; i < 78; i++)
  {
    // calculando numero de casos e atribuindo a posição corrente
    vetorCasosPorMunicipio[i].numeroDeCasos = calculaQuantidadeDeCasosComDatas(data1, data2, municipiosEspiritoSanto[i]);
    // copiando município para a posição corrente
    strcpy(vetorCasosPorMunicipio[i].municipio, municipiosEspiritoSanto[i]);
  }
  colocaEmOrdemDecrescente(vetorCasosPorMunicipio, n);
}
...
void colocaEmOrdemDecrescente(CasosMunicipio vetorCasosPorMunicipio[], int N)
{
  int i, j, aux1;
  char aux2[50];
